...
class WandBLogger:
    ...
    def _extract_config(self, config) -> Dict[str, Any]:
        """Extract config for W&B (handles both ROM and RL training)"""
        result = {}
        try:
            result['method'] = config.model.get('method', 'E2C')
            result['latent_dim'] = config.model.get('latent_dim', 128)
            result['n_channels'] = config.model.get('n_channels', 4)
            result['transition_type'] = config.config.get('transition', {}).get('type', 'linear')
        ...
        try:
            result['epochs'] = config.training.get('epoch', 0)
        ...
        try:
            result['lambda_reconstruction'] = config.loss.get('lambda_reconstruction_loss', 1.0)
            result['lambda_transition'] = config.loss.get('lambda_trans_loss', 1.0)
            result['lambda_observation'] = config.loss.get('lambda_yobs_loss', 5.0)
        except Exception:
            pass

        # RL-specific config
        try:
            rl = config.config.get('rl_model', {})
            if rl:
                result['rl_prediction_mode'] = rl.get('environment', {}).get('prediction_mode', 'state_based')
        ...
        # ROM model name (from RL dashboard)
        try:
            import builtins
            rl_cfg = getattr(builtins, 'rl_dashboard_config', None)
            if rl_cfg and 'selected_rom' in rl_cfg:
                result['rom_model_name'] = rl_cfg['selected_rom'].get('name', 'unknown')
        except Exception:
            pass
