...
class ROMWithE2C(nn.Module):
    def __init__(self, config):
        ...
        self.train_fft_loss = torch.tensor(0.0)  # FFT frequency domain loss
    ...
    def evaluate(self, inputs):
        ...
        Args:
            inputs: (X, U, Y, dt) or (X, U, Y, dt, case_indices) where X contains 3D reservoir states
                   X: List of states with shape (batch, 3, 64, 64, 4)
                   U: List of controls (batch, u_dim)
                   Y: List of observations (batch, 2*num_prob + num_inj)
                   dt: Time step (batch, 1)
                   case_indices: Optional tensor mapping batch samples to original case indices
        """
        self.model.eval()
        with torch.no_grad():
            # Extract case_indices if provided (for case-specific mask lookup)
            if len(inputs) == 5:
                X, U, Y, dt, case_indices = inputs
                model_inputs = (X, U, Y, dt)
            else:
                model_inputs = inputs
                case_indices = None
            
            predictions = self.model(model_inputs)
            # Handle variable-length prediction tuples
            if len(predictions) >= 12:
                X_next_pred, X_next, Z_next_pred, Z_next, Y_next_pred, Y, z0, x0, x0_rec, mu_list, logvar_list, Z_re_encoded = predictions
            elif len(predictions) == 11:
            ...
            # Calculate loss using updated prediction format, with optional case_indices
            t_loss = self.loss_object(predictions, case_indices=case_indices)
            ...
            self.test_fft_loss = self.loss_object.getFFTLoss()  # FFT frequency domain loss
    ...
    def update(self, inputs):
        ...
        # ===== GENERATOR UPDATE =====
        # Enable input gradients for Jacobian regularization if configured
        enable_jacobian = self.config.loss.get('enable_jacobian_loss', False)
        if enable_jacobian:
            X_list = model_inputs[0]
            if isinstance(X_list, (list, tuple)) and len(X_list) > 0:
        ...
        loss = self.loss_object(predictions, discriminator_pred, case_indices=case_indices)
        loss.backward()
        self.optimizer.step()
        ...
        self.train_fft_loss = self.loss_object.getFFTLoss()  # FFT frequency domain loss
