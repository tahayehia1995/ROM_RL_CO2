...
OUTPUT_DIR = os.path.join(_this_dir, 'timing_logs')
MODELS_DIR = os.path.join(_this_dir, 'saved_models')
DATA_DIR = os.path.join(_this_dir, 'sr3_batch_output')
PROCESSED_DATA_DIR = os.path.join(_this_dir, 'processed_data')
...
def discover_models_with_channels(model_dir: str = None) -> List[Dict[str, Any]]:
    """
...
def discover_models(model_dir: str = None) -> List[Dict[str, Any]]:
    """
...
def is_multimodal_weights(encoder_file: str) -> bool:
    """Check if encoder weights belong to a multimodal model."""
    try:
        payload = torch.load(encoder_file, map_location='cpu', weights_only=False)
        return isinstance(payload, dict) and '_multimodal' in payload
    except Exception:
        return False
...
def extract_n_channels_from_weights(encoder_file: str) -> Optional[int]:
    ...
    For multimodal models the encoder file stores two sub-encoders each with
    2 input channels; the actual data n_channels is 4.
    ...
    try:
        ...
        # Multimodal encoder payload contains sub-dicts
        if isinstance(state_dict, dict) and '_multimodal' in state_dict:
            static_sd = state_dict.get('static_encoder', {})
            dynamic_sd = state_dict.get('dynamic_encoder', {})
            n_static = static_sd.get('conv1.0.weight', torch.zeros(1,1)).shape[1] if 'conv1.0.weight' in static_sd else 0
            n_dynamic = dynamic_sd.get('conv1.0.weight', torch.zeros(1,1)).shape[1] if 'conv1.0.weight' in dynamic_sd else 0
            return n_static + n_dynamic  # Typically 2+2=4
        
        # Standard single-branch encoder
...
def extract_latent_dim_from_weights(encoder_file: str) -> Optional[int]:
    ...
    For multimodal models: static_latent_dim + dynamic_latent_dim.
...
def load_model_and_config(model_info: Dict[str, Any], config_path: str, 
    ...
    try:
        ...
        # Detect transition type from weights
        if 'transition' in model_info and model_info['transition']:
            try:
                _tsd = torch.load(model_info['transition'], map_location='cpu', weights_only=False)
                _has_sel = any(k.startswith('selector.') for k in _tsd)
                _has_nu = 'nu_layer.weight' in _tsd
                _has_alpha = 'alpha_layer.weight' in _tsd
                _has_te = any(k.startswith('trans_encoder.') for k in _tsd)
                _has_At = 'At_layer.weight' in _tsd
                _has_Vr = 'V_real' in _tsd
                _has_Ur = 'U_real_layer.weight' in _tsd
                _has_K = 'K' in _tsd and not _has_At
                _has_Act = 'A_skew_params' in _tsd
                if _has_Act:
                    _trn = 'ct_koopman'
                elif _has_K and not _has_sel:
                    _trn = 'koopman'
                elif _has_sel and _has_alpha and _has_Vr:
                    _trn = 's5'
                elif _has_sel and _has_alpha and _has_Ur:
                    _trn = 's4d_dplr'
                elif _has_sel and _has_alpha:
                    _trn = 's4d'
                elif _has_sel and _has_nu:
                    _trn = 'clru'
                elif _has_te and _has_At:
                    _trn = 'linear'
                else:
                    _trn = 'linear'
                config.config['transition']['type'] = _trn
                print(f"   Transition type detected: {_trn}")
            except Exception:
                pass
        ...
        if mm_detected:
            ...
            # Extract static/dynamic latent dims from weights
            payload = torch.load(model_info['encoder'], map_location='cpu', weights_only=False)
            if isinstance(payload, dict) and 'static_encoder' in payload:
                se = payload['static_encoder']
                de = payload['dynamic_encoder']
                if 'fc_mean.weight' in se:
                    config.config['multimodal']['static_latent_dim'] = se['fc_mean.weight'].shape[0]
                elif 'pool.proj.weight' in se:
                    config.config['multimodal']['static_latent_dim'] = se['pool.proj.weight'].shape[0]
                if 'fc_mean.weight' in de:
                    config.config['multimodal']['dynamic_latent_dim'] = de['fc_mean.weight'].shape[0]
                elif 'grid_pool.fc.weight' in de:
                    config.config['multimodal']['dynamic_latent_dim'] = de['grid_pool.fc.weight'].shape[0]
                elif 'pool.proj.weight' in de:
                    config.config['multimodal']['dynamic_latent_dim'] = de['pool.proj.weight'].shape[0]
            print(f"   🔀 Multimodal model detected")
...
def generate_predictions_with_timing(my_rom: ROMWithE2C, loaded_data: Dict[str, Any], 
    ...
    try:
        ...
        with Timer("prediction", log_dir=OUTPUT_DIR) as timer:
