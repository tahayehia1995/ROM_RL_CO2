...
class DataPreprocessingDashboard:
    ...
    def _update_states_tab(self):
        ...
        for file_path in self.available_spatial_files:
            ...
            # Create four mutually exclusive normalization checkboxes
            ...
            standard_checkbox = widgets.Checkbox(
                value=False,
                description='Standard (Z-Score)',
                style={'description_width': '150px'},
                layout=widgets.Layout(width='220px')
            )
            ...
            # Create event handlers for mutual exclusivity (now with 4 options)
            def create_mutual_exclusivity_handler_4(var_name, target_box, other_boxes):
            ...
            minmax_checkbox.observe(
                create_mutual_exclusivity_handler_4(var_name, minmax_checkbox, [standard_checkbox, log_checkbox, original_checkbox]),
                names='value'
            )
            standard_checkbox.observe(
                create_mutual_exclusivity_handler_4(var_name, standard_checkbox, [minmax_checkbox, log_checkbox, original_checkbox]),
                names='value'
            )
            log_checkbox.observe(
                create_mutual_exclusivity_handler_4(var_name, log_checkbox, [minmax_checkbox, standard_checkbox, original_checkbox]),
                names='value'
            )
            original_checkbox.observe(
                create_mutual_exclusivity_handler_4(var_name, original_checkbox, [minmax_checkbox, standard_checkbox, log_checkbox]),
                names='value'
            )
            ...
            # Add tooltips/explanations for each normalization type
            norm_help = widgets.HTML(
                value="<small style='color: #666;'>"
                      "• <b>Min-Max</b>: Scale to [0,1] range | "
                      "• <b>Standard (Z-Score)</b>: (x - mean) / std | "
                      "• <b>Log</b>: Log transform + scaling | "
                      "• <b>Original</b>: No normalization"
                      "</small>",
            ...
            # Store all four checkboxes for this property
            self.state_normalization_controls[var_name] = {
                'minmax': minmax_checkbox,
                'standard': standard_checkbox,
                'log': log_checkbox,
    ...
    def _process_data(self, button):
        ...
        with self.results_output:
            ...
            with Timer("processing", log_dir=os.path.join(_ROM_DIR, 'timing_logs')) as timer:
    ...
    def _save_normalization_parameters(self):
        ...
        save_dir = os.path.join(_ROM_DIR, 'processed_data')
    ...
    def _save_processed_data(self):
        ...
        try:
            with h5py.File(filepath, 'w') as hf:
                ...
                # Save case_indices_train (maps shuffled sample index to original case index)
                if hasattr(self, 'case_indices_train') and self.case_indices_train is not None:
                    case_data = self.case_indices_train.cpu().numpy() if hasattr(self.case_indices_train, 'cpu') else self.case_indices_train
                    train_group.create_dataset('case_indices', data=case_data)
                ...
                # Save case_indices_eval (maps shuffled sample index to original case index)
                if hasattr(self, 'case_indices_eval') and self.case_indices_eval is not None:
                    case_data = self.case_indices_eval.cpu().numpy() if hasattr(self.case_indices_eval, 'cpu') else self.case_indices_eval
                    eval_group.create_dataset('case_indices', data=case_data)
        ...
        for file_path in self.available_spatial_files:
            ...
            if hasattr(self, 'state_normalization_controls') and var_name in self.state_normalization_controls:
                ...
                if isinstance(controls, dict):
                    # New structure with four checkboxes (minmax, standard, log, original)
                    # Check for 'standard' option (may not exist in older configs)
                    standard_selected = 'standard' in controls and controls['standard'].value
                    if standard_selected:
                        self.normalization_preferences[var_name] = 'standard'
                    elif controls['log'].value:
    ...
    def _apply_sliding_window(self):
        ...
        # Unpack the return values (now includes case_indices)
        self.STATE_train, self.BHP_train, self.Yobs_train, self.STATE_eval, self.BHP_eval, self.Yobs_eval, self.case_indices_train, self.case_indices_eval = result
...
def load_processed_data(filepath=None, data_dir=None, n_channels=None):
    """
    ...
    if data_dir is None:
        data_dir = os.path.join(_ROM_DIR, 'processed_data')
    ...
    try:
        ...
        with h5py.File(filepath, 'r') as hf:
            ...
            # Load case_indices_train (maps shuffled sample to original case)
            case_indices_train = None
            if 'case_indices' in train_group:
                case_data = train_group['case_indices'][:]
                case_indices_train = torch.tensor(case_data, dtype=torch.long).to(device)
            ...
            # Load case_indices_eval (maps shuffled sample to original case)
            case_indices_eval = None
            if 'case_indices' in eval_group:
                case_data = eval_group['case_indices'][:]
                case_indices_eval = torch.tensor(case_data, dtype=torch.long).to(device)
        ...
        loaded_data = {
            ...
            'dt_train': dt_train,
            'dt_eval': dt_eval,
            'case_indices_train': case_indices_train,
            'case_indices_eval': case_indices_eval,
            'metadata': metadata,
            'norm_params': norm_params,
