...
def save_processed_data(STATE_train: List[torch.Tensor], STATE_eval: List[torch.Tensor],
                       ...
                       save_dir: str = None,
                       case_indices_train: Optional[torch.Tensor] = None,
                       case_indices_eval: Optional[torch.Tensor] = None) -> str:
    """
    ...
    Args:
        ...
        save_dir: Directory to save the file
        case_indices_train, case_indices_eval: Optional tensors mapping shuffled samples to original case indices
    ...
    try:
        ...
        with h5py.File(filepath, 'w') as hf:
            ...
            if case_indices_train is not None:
                case_data = case_indices_train.cpu().numpy() if hasattr(case_indices_train, 'cpu') else case_indices_train
            ...
            if case_indices_eval is not None:
                case_data = case_indices_eval.cpu().numpy() if hasattr(case_indices_eval, 'cpu') else case_indices_eval
