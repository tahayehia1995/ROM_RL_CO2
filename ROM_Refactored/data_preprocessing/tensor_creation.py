...
def train_split_data(SW_slt: List, SG_slt: List, PRES_slt: List, BHP_slt: List, 
    ...
    Returns:
        Tuple of (STATE_train, BHP_train, Yobs_train, STATE_eval, BHP_eval, Yobs_eval, 
                  case_indices_train, case_indices_eval)
        case_indices_train/eval: Tensor mapping each shuffled sample to its original case index
    """
    ...
    # Compute case indices for each shuffled sample
    # Each sample maps to an original case: case_index = sample_index // num_t_slt
    # This allows looking up case-specific masks during training
    case_indices_train = shuffle_ind_train // num_t_slt
    case_indices_eval = shuffle_ind_eval // num_t_slt
    ...
    # Convert case indices to tensors
    case_indices_train_tensor = torch.tensor(case_indices_train, dtype=torch.long)
    case_indices_eval_tensor = torch.tensor(case_indices_eval, dtype=torch.long)
    
    return STATE_train, BHP_train, Yobs_train, STATE_eval, BHP_eval, Yobs_eval, case_indices_train_tensor, case_indices_eval_tensor
