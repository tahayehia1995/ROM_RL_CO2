...
class EvaluationDashboard:
        ...
        if num_steps > 0:
            ...
            with open(filepath, 'w', newline='') as f:
                ...
                for t in range(num_steps):
                    ...
                    row += [max(0, act.get(k, 0)) if isinstance(act.get(k, ''), (int, float)) else act.get(k, '') for k in action_keys]
                    row += [max(0, obs.get(k, 0)) if isinstance(obs.get(k, ''), (int, float)) else obs.get(k, '') for k in obs_keys]
                    writer.writerow(row)
    ...
    def _load_from_dashboard(self):
        ...
        # Load main config (RL config has rl_model section needed by evaluation)
        import sys
        from pathlib import Path
    ...
    def _create_agent(self):
        """Create RL agent based on algorithm type in config or checkpoint."""
        from ..agent.factory import create_rl_agent, ALGORITHM_CLASSES

        # If we can detect algorithm from a checkpoint, prefer that
        algo_type = self.config.rl_model.get('algorithm', {}).get('type', 'SAC')

        # Fallback latent dim from z0 if config doesn't have it right
        if self.z0_options is not None:
            latent_dim = self.z0_options.shape[-1]
            self.config.model['latent_dim'] = latent_dim

        print(f"  Creating {algo_type} agent for evaluation...")
        self.agent = create_rl_agent(self.config, self.rl_config, rom_model=self.rom_model)
    ...
    def _update_episode_selector(self):
        """Update episode selector dropdown, defaulting to the best episode."""
        if self.evaluation_results is None:
        ...
        for ep in self.evaluation_results.all_episodes:
            ...
            if best_val is None or ep.total_npv > best_val:
                best_val = ep.total_npv
                best_case_idx = ep.z0_case_idx
        
        self.episode_selector.options = options
        if best_case_idx is not None:
            self.episode_selector.value = best_case_idx
