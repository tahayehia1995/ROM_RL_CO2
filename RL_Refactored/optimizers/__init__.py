...
from .lbfgsb_optimizer import LBFGSBOptimizer
...
def create_optimizer(config):
    ...
    Supported optimizer types:
        ...
        - 'L-BFGS-B': Quasi-Newton with limited-memory Hessian approximation
    ...
    common_args = {
        ...
        'spatial_states': config.get('spatial_states', None),
        'prediction_mode': config.get('prediction_mode', 'state_based'),
    }
    ...
    if optimizer_type == 'LS-SQP-StoSAG':
        ...
        return LBFGSBOptimizer(
            **common_args,
            max_iterations=optimizer_params.get('max_iterations', 100),
            tolerance=optimizer_params.get('tolerance', 1e-6),
            maxcor=optimizer_params.get('maxcor', 10),
            ftol=optimizer_params.get('ftol', 1e-10),
            gtol=optimizer_params.get('gtol', 1e-6),
            gradient_type=optimizer_params.get('gradient_type',
                                               stosag_params.get('gradient_type', 'spsa')),
            perturbation_size=optimizer_params.get('perturbation_size',
                                                   stosag_params.get('perturbation_size', 0.01)),
            spsa_num_samples=optimizer_params.get('spsa_num_samples',
                                                  stosag_params.get('spsa_num_samples', 5)),
        )
    
    elif optimizer_type == 'Adam':
        return AdamOptimizer(
            **common_args,
            max_iterations=optimizer_params.get('max_iterations', 200),
            tolerance=optimizer_params.get('tolerance', 1e-6),
            ...
            lr_decay=optimizer_params.get('lr_decay', 1.0),
            warmup_iterations=optimizer_params.get('warmup_iterations', 0),
            gradient_type=optimizer_params.get('gradient_type',
                                               stosag_params.get('gradient_type', 'spsa')),
            perturbation_size=optimizer_params.get('perturbation_size',
                                                   stosag_params.get('perturbation_size', 0.01)),
            spsa_num_samples=optimizer_params.get('spsa_num_samples',
                                                  stosag_params.get('spsa_num_samples', 5)),
            n_stagnation=optimizer_params.get('n_stagnation', 50),
        )
        ...
        raise ValueError(
            ...
            f"Available types: LS-SQP-StoSAG, L-BFGS-B, Adam, "
            f"Differential-Evolution, Dual-Annealing, Basin-Hopping, CMA-ES, PSO, GA, Hybrid"
        )


def run_optimization(optimizer, z0_options=None, num_steps=None, config=None):
    """
    ...
    Args:
        ...
        config: Optional dashboard config dict (used to extract ROM model name for wandb)
        
    Returns:
        OptimizationResult with optimal controls and performance data
    """
    # Initialize WandB for this optimization run
    optimizer_type = type(optimizer).__name__
    rom_name = 'unknown'
    extra_cfg = {}
    if config:
        rom_info = config.get('selected_rom', config.get('rom_model_info', {}))
        if isinstance(rom_info, dict):
            rom_name = rom_info.get('name', 'unknown')
        extra_cfg['optimizer_type_full'] = config.get('optimizer_type', optimizer_type)
        extra_cfg.update(config.get('optimizer_params', {}))
    else:
        import builtins
        rl_cfg = getattr(builtins, 'rl_dashboard_config', None)
        if rl_cfg and 'selected_rom' in rl_cfg:
            rom_name = rl_cfg['selected_rom'].get('name', 'unknown')

    optimizer.init_wandb(
        optimizer_type=config.get('optimizer_type', optimizer_type) if config else optimizer_type,
        rom_model_name=rom_name,
    ...
    if result:
        optimizer.log_wandb({
            'final/optimal_npv': result.optimal_objective,
            'final/num_iterations': result.num_iterations,
            'final/total_time_s': result.total_time_seconds,
            'final/function_evals': result.num_function_evaluations,
        })
