...
class BaseOptimizer(ABC):
    ...
    def __init__(
        ...
        spatial_states: Optional[torch.Tensor] = None,
        ...
        Args:
            ...
            spatial_states: Original full spatial states for multimodal models (optional)
            prediction_mode: 'state_based' or 'latent_based'
        """
        self.rom = rom_model
        self.config = config
    ...
    def init_wandb(self, optimizer_type: str, rom_model_name: str = 'unknown',
                   extra_config: Optional[Dict] = None):
        """Initialize a WandB run for this optimizer."""
        ...
        try:
            ...
            wb_config = {
                ...
                'rom_model_name': rom_model_name,
                'transition_type': trn_type,
                'architecture': arch,
            ...
            if extra_config:
                wb_config.update(extra_config)

            wandb.init(
                project='RL-SAC',
                name=run_name,
                tags=[optimizer_type, arch, trn_type, rom_model_name],
                config=wb_config,
                reinit=True,
            )
    ...
    def log_wandb(self, metrics: Dict[str, Any]):
    ...
    def _get_initial_spatial_state(self, z0: torch.Tensor, z0_idx: int = 0) -> torch.Tensor:
        """
        Get initial spatial state for ROM rollout.
        
        For GNN/multimodal models, uses stored original spatial states (full channels).
        For standard models, decodes from latent.
        """
        needs_original_spatial = hasattr(self.rom.model, 'encode_initial')
        
        if needs_original_spatial:
            if self.spatial_states is None:
                self._load_spatial_states_fallback()
            
            if self.spatial_states is not None:
                idx = min(z0_idx, self.spatial_states.shape[0] - 1)
                return self.spatial_states[idx:idx+1].to(self.device)
            
            raise RuntimeError(
                "GNN/Multimodal model requires original spatial states but none could be loaded. "
                "Please re-run the Optimizer Configuration Dashboard (Step 5) and click 'Apply Configuration'."
            )
        else:
            with torch.no_grad():
                return self.rom.model.decoder(z0)
    
    def _load_spatial_states_fallback(self):
        """Try to load spatial states from available sources for multimodal models."""
        # Source 1: RL dashboard's globally stored spatial states
        import builtins
        rl_spatial = getattr(builtins, 'rl_spatial_states', None)
        if rl_spatial is not None:
            self.spatial_states = rl_spatial
            print(f"   📦 Loaded spatial states from RL dashboard ({rl_spatial.shape[0]} cases)")
            return
        
        # Source 2: Load directly from H5 data files
        try:
            self._load_spatial_states_from_h5()
        except Exception as e:
            print(f"   ⚠️ Could not load spatial states from H5 files: {e}")
    
    def _load_spatial_states_from_h5(self):
        """Load spatial states from H5 data files for multimodal models."""
        import os, glob, h5py
        
        data_dir = None
        for candidate in [
            os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                         'ROM_Refactored', 'sr3_batch_output'),
            '../ROM_Refactored/sr3_batch_output/',
            'sr3_batch_output/'
        ]:
        ...
        channel_order = ['SG', 'PRES', 'PERMI', 'POROS']
        channels = []
        
        for state_name in channel_order:
            state_file = os.path.join(data_dir, f'batch_spatial_properties_{state_name}.h5')
            if not os.path.exists(state_file):
                print(f"   ⚠️ Missing H5 file for {state_name}: {state_file}")
                return
            
            with h5py.File(state_file, 'r') as f:
                data = f['data'][:, 0, :, :, :]  # First timestep of all cases
            
            if state_name in norm_params:
                p = norm_params[state_name]
                norm_type = p.get('type', 'minmax')
                if norm_type == 'log':
                    import numpy as np
                    eps = float(p.get('epsilon', 1e-8))
                    shift = float(p.get('data_shift', 0))
                    log_min = float(p.get('log_min', 0))
                    log_max = float(p.get('log_max', 1))
                    data = (np.log(data + eps - shift) - log_min) / (log_max - log_min + 1e-8)
                else:
                    mn, mx = float(p.get('min', 0)), float(p.get('max', 1))
                    data = (data - mn) / (mx - mn + 1e-8)
            
            channels.append(torch.tensor(data, dtype=torch.float32).unsqueeze(1))
        
        spatial = torch.cat(channels, dim=1)  # (num_cases, 4, D, H, W)
        self.spatial_states = spatial
        
        import builtins
        builtins.rl_spatial_states = spatial
        
        print(f"   ✅ Loaded spatial states from H5 files: {spatial.shape} ({spatial.shape[0]} cases)")
    ...
    def evaluate_objective(
        ...
        Args:
            ...
            z0_idx: Index of Z0 in ensemble (for multimodal spatial state lookup)
        ...
        # Run ROM rollout with physical controls (state-based or latent-based)
        if self.prediction_mode == 'latent':
            states, observations, rewards = self.rom_rollout_latent(z0, controls_physical, z0_idx=z0_idx)
        else:
            states, observations, rewards = self.rom_rollout(z0, controls_physical, z0_idx=z0_idx)
        ...
        if return_trajectory:
            trajectory = {
                ...
                'controls_physical': controls_physical
            }
    ...
    def evaluate_robust_objective(
        ...
        for i in range(z0_ensemble.shape[0]):
            ...
            obj, _ = self.evaluate_objective(controls, z0_single, z0_idx=i)
    ...
    def rom_rollout(
        ...
        1. Get initial spatial state (from stored data for multimodal, or decode for standard)
        ...
        Args:
            ...
            z0_idx: Index of Z0 in the ensemble (for multimodal spatial state lookup)
        ...
        current_spatial = self._get_initial_spatial_state(z0, z0_idx)
    ...
    def rom_rollout_latent(
        ...
        """
        Run ROM simulation purely in latent space using predict_latent().
        Mirrors the RL environment's latent prediction mode.
        """
        from .objective import compute_step_reward
    ...
    def generate_initial_guess(self, num_steps: int, strategy: str = 'midpoint') -> np.ndarray:
        ...
        Args:
            ...
            strategy: Initialization strategy:
                ...
                - 'custom': Use self.custom_initial_guess (set externally)
        ...
        if strategy == 'custom' and self.custom_initial_guess is not None:
            guess = np.clip(self.custom_initial_guess.flatten(), 0.0, 1.0)
            if len(guess) == n_vars:
                return guess
            print(f"   Custom guess size {len(guess)} != expected {n_vars}, falling back to midpoint")
    ...
    def decode_spatial_states(self, latent_states: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        with torch.no_grad():
            if hasattr(model, '_decode_grouped'):
                # GNN: decode via graph-based decoder
                for z in latent_states:
                    B = z.shape[0]
                    real_ids = torch.zeros(B, dtype=torch.long, device=z.device)
                    spatial = model._decode_grouped(z, real_ids, B, z.device)
                    spatial_states.append(spatial)
            else:
                for z in latent_states:
                    spatial = model.decoder(z)
                    spatial_states.append(spatial)
    ...
    def test_rom_sensitivity(self, z0: torch.Tensor):
        ...
        # Initialize spatial state (handles multimodal via stored spatial states)
        current_spatial = self._get_initial_spatial_state(z0, z0_idx=0)
        ...
        for name, control_physical in control_sets.items():
            ...
            # Denormalize and clamp negatives
            yobs_physical = self._denormalize_observations(yobs_normalized)
            yobs_physical = torch.clamp(yobs_physical, min=0.0)
