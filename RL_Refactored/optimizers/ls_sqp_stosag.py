...
class LSSQPStoSAGOptimizer(BaseOptimizer):
    ...
    def __init__(
        ...
        """
        super().__init__(rom_model, config, norm_params, device, action_ranges, init_strategy, spatial_states, prediction_mode)
    ...
    def optimize(
        ...
        # Evaluate initial objective (subsample to avoid O(N) full-ensemble cost)
        max_eval_cases = 10
        if actual_realizations > max_eval_cases:
            eval_indices = np.random.choice(actual_realizations, max_eval_cases, replace=False)
            eval_z0 = self.z0_ensemble[eval_indices]
            print(f"Evaluating initial objective on {max_eval_cases}/{actual_realizations} sampled cases...")
        else:
            eval_z0 = self.z0_ensemble
        ...
        # Evaluate final solution (subsample to avoid O(N) full-ensemble cost)
        if actual_realizations > max_eval_cases:
            eval_indices = np.random.choice(actual_realizations, max_eval_cases, replace=False)
            eval_z0 = self.z0_ensemble[eval_indices]
            print(f"Evaluating final objective on {max_eval_cases}/{actual_realizations} sampled cases...")
        else:
            eval_z0 = self.z0_ensemble
            eval_indices = np.arange(actual_realizations)
        final_obj, individual_objs = self.evaluate_robust_objective(
            optimal_controls_normalized, eval_z0
        )
        
        # Get trajectory for best realization (for visualization)
        best_local_idx = np.argmax(individual_objs)
        best_realization_idx = int(eval_indices[best_local_idx])
        z0_best = self.z0_ensemble[best_realization_idx:best_realization_idx+1]
        _, trajectory = self.evaluate_objective(optimal_controls_normalized, z0_best, return_trajectory=True, z0_idx=best_realization_idx)
    ...
    def _run_slsqp_optimization(
        ...
        def objective(x):
            ...
            obj, _ = self.evaluate_objective(controls, current_z0, z0_idx=sampled_idx)
    ...
    def _stosag_gradient(
        ...
        f0, trajectory = self.evaluate_objective(controls_base, current_z0, return_trajectory=True, z0_idx=sampled_idx)
        ...
        for i in range(n_vars):
            ...
            f_plus, _ = self.evaluate_objective(controls_plus, current_z0, z0_idx=sampled_idx)
    ...
    def _central_difference_gradient(
        ...
        for i in range(n_vars):
            ...
            f_plus, _ = self.evaluate_objective(
                x_plus.reshape(num_steps, self.num_controls), current_z0, z0_idx=sampled_idx
            )
            f_minus, _ = self.evaluate_objective(
                x_minus.reshape(num_steps, self.num_controls), current_z0, z0_idx=sampled_idx
            )
