...
class ScientificVisualization:
    ...
    def _create_spatial_analysis_tab(self):
        ...
        def refresh_spatial_tab():
            ...
            if not spatial_data or not spatial_data.get('available_timesteps'):
                content_container.children = [
                    widgets.HTML(
                        f"<p>❌ No spatial data available for episode {self.current_episode}.</p>"
                        "<p><i>To capture spatial data, enable <b>capture_spatial_states: true</b> "
                        "in the RL config before training.</i></p>"
                    )
                ]
                return
            ...
            layer_selector = widgets.Dropdown(
                options=layer_options if layer_options else [('N/A', 0)],
                value=max_layers//2 if max_layers > 0 else 0,
                description='Layer:',
                style={'description_width': 'initial'},
                layout=widgets.Layout(width='200px')
            )
    ...
    def _plot_economic_analysis(self):
        ...
        # Find break-even point (first time cumulative returns to >= 0 after going negative)
        break_even_year = None
        went_negative = False
        for i, cum_cf in enumerate(cumulative_cashflow):
            if cum_cf < 0:
                went_negative = True
            elif went_negative and cum_cf >= 0:
                break_even_year = i
                break
    ...
    def _start_rl_animation(self, button):
        ...
        def animate():
            ...
            try:
                ...
                gif_dir = _RL_DIR / "rl_animation_gifs"
    ...
    def _plot_cumulative_cashflow_evolution_for_rl_animation(self, ax, step_idx):
        ...
        try:
            ...
            # BREAK-EVEN ANALYSIS (first time cumulative returns to >= 0 after going negative)
            break_even_year = None
            went_negative = False
            for i, (year, cf) in enumerate(zip(complete_years, complete_cumulative)):
                if cf < 0:
                    went_negative = True
                elif went_negative and cf >= 0:
    ...
    def _start_episode_progression(self, button):
        ...
        def animate():
            ...
            try:
                ...
                gif_dir = _RL_DIR / "episode_progression_gifs"
                gif_dir.mkdir(exist_ok=True)
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
...
def launch_interactive_scientific_analysis(training_orchestrator=None, config=None, training_dashboard=None):
    ...
    if config is None:
        from RL_Refactored.utilities import Config
        _rl_cfg = Path(__file__).parent.parent / 'config.yaml'
        config = Config(str(_rl_cfg))
