...
class EnhancedTrainingOrchestrator:
    ...
    def finalize_episode(self, episode, total_reward=None):
        ...
        # Store complete episode data (not just the best one)
    ...
    def _store_episode_data(self, episode, episode_data):
        ...
        # Maintain maximum storage limit (never evict the best episode)
        best_ep_num = self.best_episode_data.get('episode_number', -1)
        while len(self.stored_episodes) > self.max_stored_episodes:
            oldest_episode = self.episode_order.pop(0)
            if oldest_episode == best_ep_num:
                self.episode_order.append(oldest_episode)
                continue
    ...
    def save_results(self, save_dir=None):
        ...
        variation stats so the visualization dashboard can be launched
        without re-training.
        ...
        def _tensor_to_list(obj):
            """Recursively convert tensors to lists for JSON-safe pickling."""
        ...
        payload = {
            'stored_episodes': _tensor_to_list(self.stored_episodes),
            'best_episode_data': _tensor_to_list(self.best_episode_data),
        ...
        print(f"   Episodes stored: {len(self.stored_episodes)}")
        print(f"   Best episode: {self.best_episode_data.get('episode_number', -1)}")
        return filepath

    @classmethod
    def load_results(cls, filepath):
        """Load saved training results and create a lightweight orchestrator.

        The returned object has enough data for the visualization dashboard
        to work (get_available_episodes, get_episode_data, get_best_episode_number,
        get_episode_summary_stats, etc.) without needing a config or RL env.

        Args:
            filepath: Path to a .pkl file saved by save_results().

        Returns:
            An EnhancedTrainingOrchestrator-like object ready for visualization.
        """
        ...
        class _LoadedOrchestrator:
            """Lightweight stand-in for EnhancedTrainingOrchestrator."""
            def __init__(self, data):
                self.stored_episodes = data.get('stored_episodes', {})
                # Ensure keys are ints
                self.stored_episodes = {int(k): v for k, v in self.stored_episodes.items()}
                self.best_episode_data = data.get('best_episode_data', {'episode_number': -1, 'total_reward': -np.inf})
                self.episode_order = data.get('episode_order', [])
                self.variation_stats = data.get('variation_stats', [])
            ...
            def get_best_episode_number(self):
                return self.best_episode_data.get('episode_number', -1)

            def get_best_episode_data(self):
                if self.best_episode_data.get('episode_number', -1) == -1:
                    return None
                return self.best_episode_data.copy()

            def has_best_episode_data(self):
                return self.best_episode_data.get('episode_number', -1) != -1
            ...
            def get_training_summary(self):
                if not self.variation_stats:
                    return "No variation statistics available"
                all_v = [s.get('mean_variation', 0) for s in self.variation_stats]
                return {
                    'total_episodes': len(self.variation_stats),
                    'mean_variation': sum(all_v) / len(all_v) if all_v else 0,
                    'max_variation': max(all_v) if all_v else 0,
                    'min_variation': min(all_v) if all_v else 0,
                    'final_variation': all_v[-1] if all_v else 0,
                }

            def get_spatial_data_for_episode(self, episode_number):
                ep = self.get_episode_data(episode_number)
                if ep is None or 'spatial_states' not in ep:
                    return None
                spatial_states = ep['spatial_states']
                if not spatial_states:
                    return None
                formatted = {
                    'episode_number': episode_number,
                ...
                for entry in spatial_states:
                    if isinstance(entry, dict) and 'step' in entry:
                        step = entry['step']
                        state = entry.get('spatial_state')
                        formatted['spatial_states'][step] = state
                        formatted['available_timesteps'].append(step)
                        if formatted['spatial_shape'] is None and state is not None:
                            if hasattr(state, 'shape') and len(state.shape) >= 4:
                                formatted['spatial_shape'] = state.shape[2:]
                                formatted['num_channels'] = state.shape[1]
                formatted['available_timesteps'].sort()
                return formatted if formatted['available_timesteps'] else None

        loaded = _LoadedOrchestrator(payload)
        print(f"📂 Loaded training results from: {filepath}")
        print(f"   Episodes available: {len(loaded.stored_episodes)}")
        print(f"   Best episode: {loaded.get_best_episode_number()}")
        return loaded
